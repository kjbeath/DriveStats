...
manuf = factor(manuf, levels=c("HG","Hi", "ST","TO","WD","OT"),
               labels=c("HGST","Hitachi","Seagate","Toshiba","Western Digital","Other")),
manuf = as.character(manuf)) %>%
  dplyr::mutate(groupyear = factor(year))

# survival for all disks by year
...
for (startyear in min(sumdata$year):(max(sumdata$year)-2)) {
  ...
  # remove models with no failures  
  hasfailures <- sumdata %>%
    filter(year==startyear)  %>%
    dplyr::select(model, status) %>%
    group_by(model) %>%
    summarise(hasfailures=max(status))

  print("no failures")
  print(hasfailures$model[hasfailures$hasfailures==0]) 
  
  sumdata2 <- merge(sumdata,nodrives)
  sumdata2 <- merge(sumdata2,hasfailures)
  
  sumdata2 <- sumdata2[(sumdata2$year==startyear) & (sumdata2$nodrives>2000)
                       & (sumdata2$hasfailures>0),]
